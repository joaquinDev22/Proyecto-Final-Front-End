...
export default function DejarResenaBootcamp() {
    const { id } = useParams();
    ...
    const handleSubmit = async (e: React.FormEvent) => {
        e.preventDefault();
        try {
            // TODO: Connect to CalificacionBootcampService endpoint in backend
            triggerAlert(() => {
                navigate(-1); // Volver al aula o lista
            });
        } catch (error) {
    ...
    return (
        <div className="w-full max-w-2xl mx-auto px-6 py-8 animate-fade-in-up">
            ...
            <PageHeader 
                title="Dejar Reseña del Bootcamp" 
                subtitle={`Comparte tu opinión sobre el curso #${id}`} 
            />
                ...
                <GlassCard padding="p-8" borderColor="cyan">
                    <form onSubmit={handleSubmit} className="flex flex-col gap-8">
                        ...
                        <div className="flex flex-col items-center gap-4">
                            <label className="text-white font-bold text-lg">¿Cómo calificarías este Bootcamp?</label>
            ...
            {isRendered && (
                <Alert message="¡Reseña publicada! Gracias por compartir tu opinión." type="success" isVisible={showAlert} />
            )}
        </div>
    );
}
